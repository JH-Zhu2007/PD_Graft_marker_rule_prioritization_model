...
- 支持来源追溯的 metadata 与 audit 文件。
- 最终 GitHub 展示用 12O integrated figure package。
- V18 single-panel 主图输出。
...
GitHub 展示用最终图片包位于：`figures/12O_final_integrated_package`。
...
public package 中是否检测到 06 optional context-only 组：否。
...
```text
documents/      面向 manuscript 的说明、报告或解释文件
