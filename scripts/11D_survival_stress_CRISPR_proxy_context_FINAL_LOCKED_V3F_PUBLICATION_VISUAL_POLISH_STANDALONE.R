# 11D_survival_stress_CRISPR_proxy_context_FINAL_LOCKED_V3F_PUBLICATION_VISUAL_POLISH_STANDALONE.R
# Purpose:
#   Publication-style visual polish for the LOCKED 11D final package.
#   This script DOES NOT re-parse raw data, DOES NOT re-parse Excel, and DOES NOT rerun 00-10P.
#   It only reads the validated V3E/V1/V8 tables and regenerates cleaner PDF panels.
#
# Locked scientific basis:
#   - V1 bulk/scRNA module-score tables are reused.
#   - V8 SafeHarbor-excluded CRISPR manual-column tables are reused.
#   - Final claim boundary: manual-column CRISPR proxy overlap support only.
#   - Do NOT claim validated CRISPR survival-hit genes, clinical outcome prediction,
#     graft efficacy prediction, or clinical safety prediction.
...
make_matrix_from_scores <- function(df_value) {
  ...
  # sample column: values often contain GSM or sample.
  for (jj in seq_len(ncol(df))) {
  ...
  # fallback: wide table with row labels and numeric columns.
  numeric_cols <- which(numeric_flags)
...
draw_heatmap_panel <- function(score_df, title_value, subtitle_value, footnote_value, path_value) {
  ...
  # Preserve a biologically readable order when possible.
