# ============================================================
# 11G FINAL COMPLETE STANDALONE
# Hotfix integrated as full replacement: robust gene parser without locked-binding name collision.
# PD GWAS / genetic-context support for DA/graft-related
# transcriptomic prioritisation framework
#
# User rule respected:
#   - Complete standalone script for 11G
#   - Does NOT read any previous 11G V1/V2/V3 output
#   - Does NOT use table-only patch logic
#   - Rebuilds 11G outputs from built-in conservative PD genetic
#     context catalog + current project upstream outputs when present
#   - No internet
#   - No 00-10P rerun
#   - Conservative claim boundary: genetic-context support only
#
# Output role:
#   - Provides PD genetic-context support layer for downstream 11H
#   - Does NOT claim clinical prediction, PD diagnosis,
#     causality, or validated biomarker discovery
# ============================================================
...
split_gene_cell <- function(value_obj) {
  # Robust gene-token parser using only local temporary names.
...
# ------------------------- conservative PD genetic-context catalog -------------------------
# This is a conservative seed catalog for genetic-context support.
# It is NOT a clinical diagnostic gene panel and NOT a definitive biomarker list.
# User can replace/add local GWAS files later; this module remains descriptive.
...
# ------------------------- built-in candidate marker / module gene signatures -------------------------
...
# ------------------------- optional local gene harvesting from upstream locked outputs -------------------------
# Reads only compact likely gene tables, not huge expression matrices.
# This adds evidence context if feature-importance / module-definition / CRISPR-proxy files exist.
...
harvest_candidate_files <- function(root_value) {
  ...
  # Exclude current/old 11G output folders so this complete rebuild does not read previous 11G results.
...
extract_gene_rows_from_table <- function(file_value) {
  # Completely defensive parser: a problematic upstream table is skipped, not allowed to stop 11G.
  ...
  tryCatch({
    ...
    for (col_name in gene_cols) {
      ...
      # Use explicit loops rather than lapply, so any column-level parse issue is isolated.
...
# ------------------------- optional user-provided local PD GWAS files -------------------------
# The user can place CSV/TSV files in 03_tables/11G_user_PD_GWAS_inputs or 00_data/11G_user_PD_GWAS_inputs.
# Columns can be gene, gene_symbol, nearest_gene, mapped_gene, reported_gene, hgnc_symbol, symbol.
...
if (nrow(user_gwas_df) > 0) {
  ...
  # Merge user genes into PD catalog
...
# ------------------------- master candidate gene table -------------------------
# Combine built-in module genes and harvested upstream project genes.
...
for (module_name in names(module_gene_list)) {
  ...
  # Conservative descriptive hypergeometric using approximate protein-coding universe.
  # This is supportive only; manuscript should not overclaim statistical causality.
  universe_n <- 20000
...
# ------------------------- 11H input table -------------------------
# This table is the only 11G table intended for direct downstream 11H integration.
...
# Candidate marker signature table requested for later 11H-C / 12 integration.
...
# ------------------------- figures -------------------------
# Figure A: evidence catalog summary
...
# Figure B: module support heatmap-like matrix
...
if (nrow(plot_df) > 0) {
  ...
  # column-normalize 0-1 for display
  mat_norm <- mat
