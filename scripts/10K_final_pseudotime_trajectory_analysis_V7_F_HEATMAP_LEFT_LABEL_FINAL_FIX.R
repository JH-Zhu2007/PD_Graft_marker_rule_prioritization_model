############################################################
# 10K_final_pseudotime_trajectory_analysis_V7_F_HEATMAP_LEFT_LABEL_FINAL_FIX.R
# Project: PD_Graft_Project
# Purpose:
#   Recover/export the 10K V4 final multi-timepoint pseudotime figures
#   from already generated V4 tables.
#
# Why this exists:
#   10K V4 successfully completed the matrix-safe multi-timepoint analysis,
#   but all figure exports failed because open_pdf() used
#     grDevices::capabilities("cairo")
#   whereas capabilities() is a base function, not exported by grDevices.
#
# This V7 script DOES NOT rerun matrix extraction, PCA, UMAP or pseudotime.
# It reads the completed 10K V4 tables and exports clean, separate base-R PDFs.
# Compared with V6, the ONLY intended visual change is Figure F:
#   - larger left margin
#   - smaller gene-label font
#   - y-axis title moved away from gene labels
#   - heatmap width/height adjusted to prevent Marker/gene-name overlap.
#
# Claim boundary:
#   This is graph-based transcriptomic pseudotime from GSE204796 D8/D14/D21/D28/D35.
#   It is NOT lineage tracing, NOT real fate mapping, NOT clinical prediction,
#   and NOT functional graft maturation proof.
############################################################
...
# cluster order from V4 cluster summary
...
# C: pseudotime by chronological timepoint dotrange
...
# F: heatmap from V4 heat_z table
# Intended V7 change: FINAL fix for left-side overflow.
# User-reported problem in V6: the word "Marker" at far left exceeded the page.
# Therefore V7 removes the y-axis title entirely and keeps only gene labels.
# The x-axis/plot title already make the panel self-explanatory.
# Analysis values are unchanged from V4/V5/V6.
fig_F <- file.path(OUT_FIG_DIR, "10K_V7_F_marker_trend_heatmap_clean.pdf")
tryCatch({
  # Wider landscape-like page and conservative margins.
  # No y-axis title is drawn, so nothing can spill out of the left page edge.
  ...
  # Final left-side label fix:
  # 1) gene names only, no separate y-axis title;
  # 2) slightly smaller label font;
  # 3) standard line position so labels stay inside the PDF margin;
  # 4) tick marks removed to keep the left side clean.
  axis(2,
