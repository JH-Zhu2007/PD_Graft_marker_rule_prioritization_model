...
# ============================================================
# 11J FINAL COMPLETE STANDALONE - NATURE COLOR CLEAN PUBLICATION
# ML audit / ROC-PR / feature importance review for
# DA neuron / graft-related transcriptomic prioritisation
# framework
#
# User rule respected:
#   - Complete standalone script for 11J
#   - Does NOT read any previous 11J output
#   - Does NOT use table-only patch logic
#   - May read locked upstream outputs as formal inputs:
#       09C ML outputs
#       11H integrated evidence / candidate marker signatures
#       11I module-score correlation outputs
#   - Rebuilds all 11J tables and figures from these upstream
#     locked inputs and detected project tables
#   - No internet
#   - No 00-10P rerun
#
# Scientific boundary:
#   - ML audit / marker-rule-derived model transparency only
#   - ROC/PR when score-label tables are detected
#   - Feature importance / marker-signature overlap only
...
# hard rule: do not read previous 11J output
...
# ------------------------- parse 09C prediction-like tables -------------------------
...
if (length(file_09c_candidates) > 0) {
  for (idx_file in seq_along(file_09c_candidates)) {
    ...
    # Avoid feature-importance tables being misread as prediction tables.
...
# ------------------------- parse 09C feature importance tables -------------------------
feature_list <- list()
for (idx_file in seq_along(file_09c_candidates)) {
  file_value <- file_09c_candidates[idx_file]
  data_value <- read_table_safe(file_value)
...
if (nrow(feature_raw_df) > 0) {
  # Aggregate across files by max absolute importance.
  ...
  # Conservative fallback: if no explicit 09C feature table detected, use module-level 11I and marker contexts
  # as audit placeholders, clearly marked as fallback and not real feature importance.
...
# ------------------------- marker/evidence overlap annotation -------------------------
...
# Annotate module-like feature names.
...
# ------------------------- figures -------------------------
# FigA input audit only, claim-boundary text is kept in tables/report, not inside the panel
...
# FigB ROC-PR curves with clean axes, compact legend, and PR baseline
...
# FigC feature-transparency / marker-overlap audit without bottom explanatory text
...
# FigD integrated ML audit summary without interpretation paragraph
