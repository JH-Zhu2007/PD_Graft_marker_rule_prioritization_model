# ============================================================
# 10C_FINAL_V16_F5B_CONTEXT_CLUSTER_SIZE_LOCK.R
# ============================================================
#
# PD_Graft_Project — 10C 一键完整自动版（V10 explicit-local-module-paths module-safe 修正版）
#
# 适用场景：
#   - 你已经完成 10A 和 10B。
#   - 你不想再手动 rescue、手动填 override。
#   - 你希望 10C 自动扫描 04_figures，自动匹配主图/补图 source，
#     自动复制、自动生成 manifest、legend draft、claim-boundary note 和 10D brief。
#
# 重要说明：
#   - 本脚本不重跑任何生物学分析。
#   - 不重新训练 ML。
#   - 不重新定义阈值。
#   - 不修改任何 PDF 内容。
#   - 它只是锁定和复制已有 figure source。
#
# 匹配策略：
#   1. 先使用 strict frozen-version matching。
#   2. strict 找不到或多候选时，自动进入 loose scoring rescue。
#   3. 对每个 panel 自动选择最高分候选。
#   4. 若没有任何候选，才停止。
#   5. 所有 auto-rescue 选择都会写入审计表，供之后 10D 前复核。
#
# 输出目录：
#   D:/PD_Graft_Project/09_manuscript/10C_final_V16_F2E_S7A_HALLMARK_BARPLOT_F5B_CLUSTER_SIZE/
#   D:/PD_Graft_Project/03_tables/10C_final_V16_F2E_S7A_HALLMARK_BARPLOT_F5B_CLUSTER_SIZE/
#   D:/PD_Graft_Project/06_reports/10C_final_V16_F2E_S7A_HALLMARK_BARPLOT_F5B_CLUSTER_SIZE/
#
# 需要 R 包：
#   install.packages(c("data.table", "digest", "pdftools"))
#
# 运行：
#   source("10C_FINAL_V16_F5B_CONTEXT_CLUSTER_SIZE_LOCK.R")
#
# ============================================================
...
# 主图 source 最好是单页；一键版不强制停止，但会标记。
REQUIRE_SINGLE_PAGE_MAIN_SOURCE <- FALSE

# PDF 最小文件大小。
MIN_PDF_SIZE_BYTES <- 1000

# 低置信度阈值。低于这个分数仍会选择 top1，但会标记 low_confidence。
...
# V4 关键安全阀：
#   auto-rescue 只能在预期模块目录中选图。
#   例如 09I 的 GSE243639 panel 绝不能从 09E / 09F / 09G / 10A 里拿图。
REQUIRE_MODULE_PREFIX_FOR_AUTO_RESCUE <- TRUE

# V4 关键安全阀：
#   对特定 panel 增加必须出现的语义关键词，避免用相近但错误的图替代。
...
# ============================================================
# 4B. V13 supplement de-dup local module paths
# ============================================================

# V13 不使用压缩包，不复制外部文件。
# 它直接使用你本地 04_figures 里的真实模块文件夹：
#   09A_scRNA_cell_state_proportion_final_V6_pdf
#   09C_primary_reduced_feature_weak_label_ML_V4_FULL_PUBLICATION_LAYOUT_pdf
#
# 关键修正：
#   之前 MIN_PDF_SIZE_BYTES=5000，会把 4.5 KB / 4.8 KB 的合法小 PDF 排除；
#   现在降为 1000，并且对 F1E/F3B/F3C/S8B 使用 explicit local path priority。
...
# ============================================================
# 4C. V13 explicit path priority for 09A/09C plus supplement de-dup
# ============================================================
...
v13_explicit_source_patterns <- data.frame(
  ...
  preferred_basename_regex = c(
    ...
    # Priority 1 de-dup rule:
    # S2B must use pure projection competence score, not DA_projection_competence combined score.
    "^GSE204796.*V19_UMAP_score_projection_competence_composite_score(\\(1\\))?\\.pdf$",
    # Priority 2 optional de-dup rule:
    # S10B should prefer a 09I diagnostic if present; fallback is handled below.
...
# V15 explicit S7A rule:
# Supplementary Figure 7 must be Hallmark GSEA barplot.
# It must not silently fall back to spotplot/dotplot/bubble plot.
...
find_preferred_candidates <- function(plan_row) {
  ...
  # S2B hard protection:
  # Avoid accidentally selecting the combined DA_projection score again.
  ...
  # S10B preference order:
  # 1. true 09I diagnostic/cluster size/coverage/alignment/prediction summary;
  # 2. if those do not exist, retain 09I marker-targeted import summary rather than selecting non-09I figures.
  ...
  # S7A hard protection:
  # For Supplementary Figure 7, only barplot is allowed.
  # Do not select dotplot/spotplot/bubble plot even if it scores well.
...
# 诊断：列出 V13 明确优化的 target files 是否被 inventory 读到，以及是否通过 integrity。
...
# V13 final fix:
# 写出完整 PDF inventory。V11 最后 verification 失败就是因为这里被诊断表写出覆盖掉了。
atomic_write_csv(
...
score_loose_candidates <- function(plan_row) {
  ...
  # V4 module-safe hard filters
  # 这一步是为了防止 V3 中出现的明显错配：
  #   F5A/F5B/F5C/F5E 等 09I panel 被 10A/09E/09F/09G 图替代；
  #   F3B 的 09C internal CV 被 09H negative-control 图替代；
  #   F1B 允许 GSE178265/GSE132758/GSE204796 中的代表性 V19 cluster UMAP，但不能被 annotation/score 图替代。
  module_locked <- if (REQUIRE_MODULE_PREFIX_FOR_AUTO_RESCUE) {
    # V5 放宽“目录必须含模块号”的限制：
    # 如果用户把恢复的 PDF 暂时放在 04_figures 根目录，
    # 只要文件名本身以 09A/09C/09I 等模块号开头，也可视为 module-safe。
    # 08A 的 V19 UMAP 文件常以 GSE 编号开头，因此允许 filename 中含 V19 且为 UMAP。
...
# ============================================================
# 10C. V15 content-based Hallmark barplot selection
# ============================================================
...
is_hallmark_barplot_like <- function(path, basename_value, relative_value) {
  ...
  # Accept if:
  #   1. it is a Hallmark/GSEA source;
  #   2. it is not dot/spot/bubble-like;
  #   3. either filename/path says barplot OR the text looks like a Hallmark NES plot without dot-size legend.
  has_hallmark &&
    !dot_like &&
    (
      has_bar_name ||
...
# ============================================================
# 10D. V16 Figure 5B / S10B 09I source replacement
# ============================================================
...
for (i in seq_len(nrow(all_plan))) {
  ...
  # V16 hard source override:
  # F5B must be context cluster size barplot.
  # S10B is reassigned to marker overlap diagnostic so that F5B/S10B are not duplicates.
  ...
  # V16 final hard veto:
  # F5B must be cluster-size barplot, not marker overlap.
  # S10B must be marker overlap diagnostic, not cluster size.
  ...
  # V15 final hard veto:
  # F2E and S7A must remain true Hallmark GSEA barplots after all selection logic.
...
# V8 local folder diagnostic for 09A/09C/Figure 5 unresolved cases
