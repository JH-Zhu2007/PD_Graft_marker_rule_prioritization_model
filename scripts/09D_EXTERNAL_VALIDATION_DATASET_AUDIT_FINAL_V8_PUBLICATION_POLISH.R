# 09D~09D_EXTERNAL_VALIDATION_DATASET_AUDIT_FINAL_V8_PUBLICATION_POLISH.R ----
#
# ============================================================
# 09D_EXTERNAL_VALIDATION_DATASET_AUDIT_FINAL_V8_PUBLICATION_POLISH.R
# ============================================================
# 09D：External validation dataset eligibility audit
#
# V8 publication polish：
#   这是完整 09D，不是单独修图脚本。
#   逻辑继承 V6/V7 的 curated strict roles，只进一步优化投稿图排版。
#
# 保留核心结论：
#   - GSE183248：tier1 primary frozen validation candidate
#   - GSE184950 / GSE243639：tier1 disease-context validation candidates
#   - GSE128040 / GSE148434：tier2 backup/mechanistic-context candidates
#
# V8 图形改进：
#   1. eligibility score 图右侧不再显示超长 tier 字符串。
#      改成短标签：
#        Primary
#        Context
#        Backup
...
#        Exclude
#
#   2. “Recommended score threshold = 8” 放在图上方外边距，
#      不放在图内，不遮挡柱子。
#
#   3. 增加右侧留白和短标签位置控制，避免文字出界。
#
#   4. role summary 图也改成短标签：
#        Primary frozen validation
#        Disease-context validation
#        Backup/mechanistic context
#
#   5. 输出表格仍保留完整 tier / role 字段，图里只用短标签。
#
# 严谨性原则：
#   - 00–09C 冻结，不回头修改。
#   - 09D 只做 external validation dataset audit。
#   - 不分析表达矩阵。
#   - 不根据外部结果改 marker / threshold / model。
#
# 成功标志：
#   ✅ 09D external validation dataset eligibility audit FINAL V8 PUBLICATION POLISH 完成。
# ============================================================
#
...
# 00–09C 主分析已经用过的数据集，09D external validation 必须排除。
...
# 09D 内置候选外部验证数据集。
# 注意：这里是候选，不是自动最终选择；脚本会做 eligibility audit。
...
# V6：预定义 curated role table。
# 这不是根据 09E 表达结果挑选，而是在 09E 之前仅基于 GEO 标题/metadata/生物学场景锁定用途。
# 目的：防止 V5 那样把所有 broadly PD-related single-cell datasets 都判成 primary。
...
# 最低要求：作为 primary external validation candidate，建议至少满足这些分数。
...
has_any_pattern <- function(text, patterns) {
  # V3: text can be a vector from GEO metadata. Collapse it first so that
  # each grepl() returns exactly one TRUE/FALSE value.
...
parse_gse_soft_fallback <- function(gse_id, cache_dir) {
  ...
  # V6: If cached SOFT is corrupted/incomplete, delete it, re-download once, and retry.
...
# ============================================================
# 4. 冻结 00–09C framework manifest
# ============================================================
...
for (acc in candidate_dt$gse_id) {
  ...
  # 优先 GEOquery；如果未安装或单个 accession 失败，则 fallback 到 GEO SOFT。
  gse_obj <- NULL
...
# V4 journal-level role/evidence classification.
...
# V5 strict journal-level classification.
# The goal is to prevent every broadly PD-related single-cell dataset from becoming "primary".
...
# V6：curated strict role overrides the overly broad automated primary calls.
# The automated evidence is still kept in the output, but final tier is role-locked before 09E.
...
# V8 publication layout:
# - long tier names are not printed in the figure
# - threshold text is placed in the top margin
# - right margin is enough for short tier labels
...
# V8: short tier label, fixed x position, no clipping / no long-string overflow.
text(
  x = xmax_score + 0.42,
  y = bp,
