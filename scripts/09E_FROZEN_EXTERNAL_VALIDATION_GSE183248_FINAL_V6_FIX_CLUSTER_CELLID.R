# 09E~09E_FROZEN_EXTERNAL_VALIDATION_GSE183248_FINAL_V6_FIX_CLUSTER_CELLID.R ----
#
# ============================================================
# 09E_FROZEN_EXTERNAL_VALIDATION_GSE183248_FINAL_V6_FIX_CLUSTER_CELLID.R
# ============================================================
# 完整 09E 脚本，一次性运行：
#
#   Part 1：GSE183248 external data import + frozen scoring + whole-dataset prediction
#           继承 09E V2 的修复：
#             - 修复 external_group duplicated column
#             - 修复 processed matrix negative values / log1p NaNs
#
#   Part 2：metadata grouping recovery + unsupervised external cluster-level validation
#           整合原 09E3：
#             - 尝试恢复 biological groups
#             - 如果无法恢复，就做 external cluster recovery
#             - 按 cluster 重新汇总 scores
#             - 对每个 external cluster 输出 ideal-like / safety-risk probabilities
#
#
# V6 修正：
#   V3 的 Part 1 已经成功，但 Part 2 重新 readRDS external matrix 时，
#   因本地 RDS 兼容/损坏问题报 ReadItem error。
#   V4/V5 在同一次完整运行中优先直接使用 Part 1 内存里的 external_counts、score_dt、external_meta，
#   不再依赖重新读取 external matrix RDS；只有单独运行 Part 2 时才 fallback 读取本地文件。
#   V5 进一步修正：
#     V4 误把 GSE183248_Metadata.csv 一类 metadata 文件当作 expression matrix 导入，
#     导致出现一个假 external group：GSE183248_Metadata（n=11）。
#     V5 在 text matrix candidate 阶段明确排除 metadata / annotation / phenotype / sample-info 文件，
#     并在导入后再次过滤 metadata-like sample，只保留真正 expression matrix。
#     因此后续 biological grouping 不会再被 Metadata 假分组误导；如果没有真实 sample/condition，
#     会自动进入 unsupervised external cluster recovery。
#
#   V6 进一步修正：
#     V5 base section 已经正确排除 Metadata，并成功只保留 Raw_data：17975 genes / 4495 cells。
#     但 unsupervised cluster recovery 中 PCA/kmeans 返回对象缺少 cell_id，
#     导致 merge.data.table 报 y missing [cell_id]。
#     V6 在 clustering 前后强制保留 cell_id：
#       - dense matrix rownames = colnames(expression matrix)
#       - PCA score rownames = dense rownames
#       - cluster_dt 明确输出 cell_id
#
#
# 总脚本整合规则：
#   后面整合 00–09E 时，只放这个 09E V4 FULL。
#   不再单独放 09E V1、09E V2、09E3。
#
# 成功标志：
#   ✅ 09E frozen external validation GSE183248 FINAL V6 FIX CLUSTER CELLID 完成。
# ============================================================
...
# 是否自动下载 GEO supplementary files
DOWNLOAD_SUPPLEMENTARY <- TRUE

# 避免误下极大文件。GSE183248 通常不应特别大；这里留宽一些。
MAX_SUPPLEMENTARY_FILE_MB <- 5000

# 如果检测到多个 matrix，最多导入几个；GSE183248 外部验证目标是整体数据，通常会合并。
MAX_MATRICES_TO_IMPORT <- 20

# 最少 marker overlap。低于这个阈值，该 signature 只记录，不用于强解释。
...
# group summary 优先使用的 metadata 字段
...
download_file_safe <- function(url, dest, max_mb = MAX_SUPPLEMENTARY_FILE_MB) {
  ...
  # Try HEAD-like size check if possible.
  # Base R cannot reliably get remote size for all servers; we still keep a max_mb field in audit.
  res <- tryCatch({
...
normalize_counts_log1p_cpm <- function(counts) {
  # V2 robust normalization decision:
  # Some GEO processed matrices are already normalized/log-like and may contain negative values.
  # Applying log1p CPM to such matrices creates NaNs and is methodologically wrong.
  x <- counts@x
  ...
  # Case 1: negative values => already processed expression; do not log1p.
  ...
  # Case 2: non-negative but small range / not integer-like => likely already processed.
  ...
  # Case 3: raw count-like matrix => log1p CPM.
...
# ============================================================
# 4. 读取 09D V8 decision / 确认 primary candidate
# ============================================================
...
# ============================================================
# 5. 下载/整理 GSE183248 supplementary files
# ============================================================
...
if (length(supp_files) == 0) {
  # fallback to common GEO raw tar
...
# V5 safeguard: remove metadata-like candidates that may slip through because "metadata" contains "data".
...
# ============================================================
# 7. 导入 external expression matrix
# ============================================================
...
for (i in seq_len(n_import)) {
  ...
  # V5 safeguard: never import metadata-like text tables as expression matrices.
  ...
  if (inherits(res, "error")) {
    ...
    # V5 safeguard: if a text table was still imported but sample_id is metadata-like, exclude it.
...
# Remove duplicated gene symbols by summing duplicated rows
...
# ============================================================
# 10. External group-level summary
# ============================================================
...
# V2 fix:
# Do NOT manually create external_group inside j when using by = external_group.
# Otherwise data.table creates duplicated column name [external_group].
...
# Add pct > group-wise median baseline for major score columns
...
# ============================================================
# 11. 构建 external reduced feature table for frozen predictor
# ============================================================
...
# Convert score columns to marker_* columns where possible
for (sc in score_cols) {
  base <- sub("^score_", "", sc)
...
# Try to construct A9_minus_A10_score_05A from available score columns
...
# ============================================================
# 12. Frozen predictor application
# ============================================================
...
if (length(prediction_outputs) > 0) {
  ...
  # Exploratory class from logistic probabilities only if both tasks available
...
# ============================================================
# 14. Method note / report / verification
# ============================================================
...
# ============================================================
# 16. 09E subgroup enhancement: metadata grouping + cluster recovery
# ============================================================
# 下面这一段原来是 09E3，现在已经整合进 09E V4 FULL。
# 它会直接读取上面 09E V3 已经生成的 external matrix / cell scores / metadata，
# 不需要单独运行 09E3，也不需要单独保存 09E3 到总脚本。
# ============================================================
...
# 外部 cluster 数。GSE183248 大约 4506 cells，8 个 cluster 比较适合作为外部验证分组。
DEFAULT_K_CLUSTERS <- 8

# 用于 PCA/clustering 的高变基因数量
N_HVG_FOR_EXTERNAL_CLUSTERING <- 2000

# PCA 维度
N_PCS <- 30

# 最小 cluster 细胞数；低于这个数标记为 small_cluster，但不删除
...
make_cluster_from_matrix <- function(mat, n_hvg = N_HVG_FOR_EXTERNAL_CLUSTERING, n_pcs = N_PCS, k = DEFAULT_K_CLUSTERS) {
  ...
  # V6: ensure unique and non-empty cell ids before PCA/clustering.
  ...
  # V6 critical fix:
  # Matrix transpose usually preserves rownames, but some conversions/PCA methods can drop them.
  # Force rownames here and again after PCA.
  ...
  # V6 critical fix: force PCA score rownames to match cell ids.
  ...
  # V6 hard validation before merge.
...
# ============================================================
# 4. 读取 09E base section 输出
# ============================================================

stamp("读取 09E base section 输出 / 内存对象。")

# V4 fix:
# In a FULL integrated 09E run, Part 1 has already created these objects in memory:
#   external_counts, score_dt, external_meta
# V3 failed because Part 2 tried to re-read the saved RDS and hit a local RDS compatibility/corruption issue.
# Therefore V4 preferentially uses in-memory objects and only falls back to files when necessary.
...
# Infer from all available text fields
...
# First, use a meaningful existing metadata column if it has >=2 groups and covers >=80% cells.
...
# ============================================================
# 6. 如果无可靠 biological grouping，则做 cluster-level recovery
# ============================================================
...
# ============================================================
# 7. group/cluster-level score summary
# ============================================================
...
# Add marker profile: rank high scores within each external group
...
# ============================================================
# 8. 构建 cluster-level reduced feature table
# ============================================================
...
# Create marker_* from score_* columns
for (sc in score_cols) {
  base <- sub("^score_", "", sc)
  marker_name <- paste0("marker_", base)
  if (!marker_name %in% names(external_features)) {
...
# Try to construct A9_minus_A10_score_05A
a9_cols <- names(external_features)[grepl("A9", names(external_features), ignore.case = TRUE)]
a10_cols <- names(external_features)[grepl("A10", names(external_features), ignore.case = TRUE)]
...
# ============================================================
# 9. Apply discovery-only frozen predictor to external clusters
# ============================================================
...
# Scale columns only if more than one group
if (nrow(mat) > 1) {
  mat_scaled <- scale(mat)
} else {
  mat_scaled <- mat
