# 09B~09B_ML_READY_DATASET_AND_LEAKAGE_AUDIT_FINAL_V4_FULL_FIXED_LAYOUT.R ----
#
# ============================================================
# 09B_ML_READY_DATASET_AND_LEAKAGE_AUDIT_FINAL_V4_FULL_FIXED_LAYOUT.R
# ============================================================
# 09B 完整版：ML-ready dataset construction + leakage/circularity audit
#
# 这是 09B 的完整整合版：
#   - 包含 V3 的完整计算逻辑
#   - 包含 V2 的 09A object composition 重复 key merge 修复
#   - 包含 V3 的 data.table logical filter 修复
#   - 集成 V4 的两个 fixed-layout PDF 图
#
# 重点修复的图：
#   1. 09B_feature_leakage_category_summary.pdf
#      长标签不再超出图像边界。
#   2. 09B_top_feature_label_association_audit.pdf
#      左侧 feature label 自动缩短/换行，不再超出图像边界。
#
# 严谨性原则：
#   - 不训练模型
#   - 05B label 是 rule-derived marker-rule-derived label，不是实验 ground truth
#   - object-level composition 只作为 audit/context，不进入训练特征
#   - direct label-defining features 明确标记为 critical circularity
#   - 09C primary ML 推荐使用 reduced_non_direct feature tables
#
# 成功标志：
#   ✅ 09B ML-ready dataset and leakage audit FINAL V4 FULL FIXED-LAYOUT 完成。
# ============================================================
#
...
classify_feature_leakage <- function(feature_name) {
  ...
  # 直接定义 label 或来自 05B contrast class 的字段
  ...
  # 与 label 语义强相关的 marker category / signature
  ...
  # A9/A10 是 biological identity，不直接定义 ideal vs safety，但仍属于相关 signature
...
# ============================================================
# 5. 标准化 marker-rule-derived labels
# ============================================================
...
# ============================================================
# 6. 合并 09A object-level composition 信息，仅用于审计，不用于训练
# ============================================================

stamp("合并 09A object-level composition 信息，标记为 label-derived composition。")

# V2 重要修复：
# 09A object priority table 理论上应为 dataset + object_id 一行，
# 但如果 object_id 在某些来源中重复，直接 merge 会触发 cartesian join。
# 因为 09A composition 在 09B 仅作为 audit/context，不用于训练，
# 所以这里先做唯一化并输出重复 key audit。
...
if (nrow(object09A) > 0 && all(c("dataset", "object_id") %in% names(object09A))) {
  ...
  if (length(comp_cols) > 0) {
    ...
    # 数值列：同一 dataset/object_id 多行时取均值。
    # 这只用于 audit/context，不参与模型训练。
...
# ============================================================
# 7. Feature dictionary and leakage risk classification
# ============================================================
...
# ============================================================
# 8. Label definitions and training tables
# ============================================================
...
# ============================================================
# 10. Class balance and LODO feasibility
# ============================================================
...
# Top feature-label association plot - V4 fixed layout
...
# Feature category summary plot - V4 fixed layout
...
# ============================================================
# 13. Python manifest / method note / report
# ============================================================
