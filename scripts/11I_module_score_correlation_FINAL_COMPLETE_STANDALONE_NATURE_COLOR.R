...
# ============================================================
# 11I FINAL COMPLETE STANDALONE - NATURE COLOR
# Module-score correlation for DA neuron / graft-related
# transcriptomic prioritisation framework
#
# User rule respected:
#   - Complete standalone script for 11I
#   - Does NOT read any previous 11I output
#   - Does NOT use table-only patch logic
#   - Rebuilds 11I from original/current project RDS objects when present
#   - Optionally annotates candidate marker signatures using locked 11H table
#     when present, but does not depend on any previous 11I result
#   - No internet
#   - No 00-10P rerun
#
# Output role:
#   - Quantifies module-score correlation structure across state-level
#     transcriptomic module scores
#   - Supports 11H integrated evidence tier and downstream 11J ML audit
#   - Keeps conservative claim boundary
#
# Scientific boundary:
#   - Transcriptomic module-score correlation only
#   - No clinical prediction
#   - No validated biomarker claim
#   - No true lineage/projection/functional integration proof
# ============================================================
...
# ------------------------- Nature-style color helpers -------------------------
# Palette chosen for clean journal-style figures:
#   deep navy / teal / green for identity and audit
#   orange-red for positive correlation and risk context
#   blue-white-red diverging scale for correlation heatmaps
nature_palette <- list(
  ink = "#1D1D1F",
  muted = "#5F6368",
  grid = "#E6E8EB",
...
# Backward-compatible name used by the script, now returning Nature-style colors.
...
# Backward-compatible name used by the script, now returning Nature-style diverging colors.
...
# ------------------------- optional 11H marker table annotation -------------------------
...
get_matrix_from_object <- function(obj_value) {
  # Returns genes x cells matrix-like object or NULL.
  mat_value <- NULL

  # Seurat object - use namespace only if available, avoid hard dependency.
...
compute_module_scores <- function(mat_value, module_list) {
  # Return data frame: cells x modules.
  ...
  # collapse duplicate upper symbols by first occurrence for speed
...
# Prefer annotated/QC/scored objects if many files exist.
...
if (length(rds_files) > 0) {
  for (idx_file in seq_along(rds_files)) {
    ...
    # Skip huge objects only if too large for safe local run; sample cells deterministically.
...
# Remove duplicated identical object/state rows conservatively.
...
# Keep states with at least two non-NA module scores.
...
# Drop modules with insufficient finite values or no variance.
...
# Candidate marker signature annotation by modules.
...
# ------------------------- figure A: object scan and claim boundary -------------------------
...
# ------------------------- figure B: module correlation heatmap -------------------------
# Order modules by hierarchical clustering if possible.
...
# ------------------------- figure C: identity-risk axis landscape -------------------------
...
if (sum(ok) >= 3) {
  ...
  # label top states by priority balance
...
# ------------------------- figure D: top module correlations -------------------------
