# ================================================================
# 10O advanced source figure lock V8
# REVIEW-ACCEPTED ASSEMBLY-READY COMPLETE STANDALONE VERSION
# ================================================================
# Purpose:
#   1. Read 10M advanced figure plans and 10N storyline/source-lock instructions.
#   2. Scan all PDF figure candidates under the project.
#   3. Source-lock every accepted main/supplementary panel for 10P assembly.
#   4. Preserve locked final modules:
#        - 10K final = V4 analysis + V7 figure export heatmap left-label final fix
#        - 10L final = V2 signature-priority inference demo, NOT serialized 09C model prediction
#   5. Convert existing ambiguous-but-present source PDFs into review-accepted assembly-ready rows.
#   6. Keep warnings so 10Q visual audit remains mandatory.
#
# This script does NOT generate images and does NOT rerun analysis.
# It only builds source-lock manifests for the next 10P assembly step.
# ================================================================
...
standardize_plan_10o <- function(dd, role_name) {
  ...
  # Fallback figure/panel names if missing.
  for (ii in seq_len(n)) {
    ...
    if (nchar(out$panel_id[ii]) < 1) {
      # 24 main rows are expected as 9 figures with <=3 panels; this fallback is only cosmetic.
...
# -----------------------------
# Locked final-module override map
# -----------------------------
...
score_candidate_10o <- function(row_hint, fig_id, panel_id, source_mod, pdf_path, pdf_name) {
  ...
  # Panel-letter matching for final 10K/10L file names.
...
select_source_for_row_10o <- function(one_row) {
  ...
  # Explicit final overrides for 10K Figure 6 and 7.
  ...
  # Explicit final overrides for 10L supplementary/GitHub demo.
...
# -----------------------------
# Top candidate alternatives for review
# -----------------------------
alt_list <- list()
alt_idx <- 1
for (ii in seq_len(nrow(lock_rows))) {
...
# -----------------------------
# Claim-boundary crosswalk from 10N
# -----------------------------
