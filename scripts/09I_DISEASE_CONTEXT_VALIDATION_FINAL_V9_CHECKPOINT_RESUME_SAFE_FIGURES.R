# 09I~09I_DISEASE_CONTEXT_VALIDATION_FINAL_V9_CHECKPOINT_RESUME_SAFE_FIGURES.R ----
#
# ============================================================
# 09I_DISEASE_CONTEXT_VALIDATION_FINAL_V9_CHECKPOINT_RESUME_SAFE_FIGURES.R
# ============================================================
# V9 修正：
#   V8 已成功完成最耗时的 marker-targeted import：
#     scanned_genes=33525
#     marker_genes_kept=121
#     cells=83484
#     marker_nnz=1180554
#   但在出图阶段因为 ggplot2::margin() 被其他包/函数遮蔽，触发：
#     Error in margin.default(8, 12, 8, 12) : 12 is not a factor
#
#   V9 做两件事：
#     1. 所有 plot margin 显式写成 ggplot2::margin()，避免函数遮蔽。
#     2. 增加 checkpoint/resume：
#        - 如果 V9 自己已有 target_counts/target_norm RDS，直接读取，不重新导入。
#        - 如果检测到 V8 已保存的 RDS 和 audit，会自动复制/读取 V8 checkpoint。
#        - 只有找不到任何 checkpoint 时，才重新执行 marker-targeted import。
#
#   这样以后脚本中途出错，也不会浪费已经完成的重型导入结果。

# 09I：Disease-context validation on GSE243639 using marker-targeted local import
#
# V9 修正：
#   V7 已经证明本地 GSE243639_Filtered_count_table.csv.gz 可以读取到大量内容，
#   但 gzip 文件末尾仍提示 invalid / incomplete compressed data。
#   V7 试图构建全量 sparse matrix，文件过大且末尾残缺导致整体 import 判定失败。
#
#   V9 改为更稳、更适合 09I disease-context supplement 的策略：
#     1. 完全 local-only，不联网、不下载。
#     2. 不再构建全量 3万+ genes x cells sparse matrix。
#     3. 逐块扫描整个 count table：
#          - 计算每个 cell 的全基因 library size
#          - 只保存 frozen 04A marker genes 对应的表达值
#     4. 对 marker genes 做 log1p(CPM) normalization。
#     5. 计算每个 cell 的 frozen marker-category scores。
#     6. 基于 18 个 frozen signature scores 做 disease-context signature-space clustering。
#     7. 将 09B V4 reduced non-direct frozen predictors 应用于 context clusters。
#     8. 如果文件尾部有 incomplete gzip warning，会记录在 audit 中，但不再让整个 09I 失败。
#
# 严谨性定位：
#   V9 是 disease-context marker-targeted validation，不是 full-transcriptome raw reanalysis。
#   不作为 primary graft validation。
#   不证明临床安全、治疗效果、真实投射或功能整合。
#
# 输出：
#   D:/PD_Graft_Project/03_tables/09I_disease_context_validation_V9_CHECKPOINT_RESUME_SAFE_FIGURES/
#   D:/PD_Graft_Project/04_figures/09I_disease_context_validation_V9_CHECKPOINT_RESUME_SAFE_FIGURES_pdf/
#
# 成功标志：
#   ✅ 09I disease-context validation FINAL V9 CHECKPOINT RESUME SAFE FIGURES 完成。
# ============================================================
...
# V9 checkpoint / resume settings
RESUME_FROM_EXISTING_CHECKPOINTS <- TRUE

# Older V8 checkpoint location. V9 can reuse it once, then writes V9 outputs.
...
read_marker_target_counts_with_library <- function(path, target_genes, chunk_n_genes = TARGET_IMPORT_CHUNK_N_GENES) {
  ...
  repeat {
    ...
    # Full-library size across all parsed genes.
    ...
    # If warning occurred, stop after processing whatever lines were returned.
    if (isTRUE(rr$warned)) {
      break
    }
  }
  ...
  # Sum duplicate marker genes if any.
...
# ============================================================
# 5. Marker-targeted local import
# ============================================================
...
# 1. Prefer existing V9 checkpoint
...
# 2. Reuse V8 checkpoint if V9 checkpoint not available
...
# 3. If no checkpoint exists, perform heavy marker-targeted import
if (!checkpoint_loaded) {
...
# ============================================================
# 6. Scoring and signature-space clustering
# ============================================================
...
# Add cell-level A9-A10 if possible
...
# ============================================================
# 10. Key findings / method note / report
# ============================================================
