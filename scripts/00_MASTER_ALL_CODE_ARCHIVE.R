#
# PD_Graft_Project 00-to-05B ALL-IN-ONE MERGED RAW SCRIPT
#
# 说明：本文件仅按顺序合并用户提供的 00 到 05B 脚本。
# 除本总标题和每段 BEGIN/END 分隔注释外，未修改各脚本内部代码、参数或逻辑。
#



# 00~00_manual_data_audit_v2(1).R ----
...
# END 00_manual_data_audit_v2(1).R


# 00A5~00A5_fast_GEO_download_fixed_7899(1).R ----

#
# 00A5_fast_GEO_download_fixed_7899.R
# 项目：PD干细胞治疗“功能身份—疾病脆弱性悖论”
# 用途：使用aria2多连接断点续传，并强制通过Clash Verge本地代理下载剩余GEO文件
#
# 适用：
#   - Windows
#   - 当前已有 .part 文件
#   - NCBI单连接速度慢、经常 schannel/server closed abruptly
#
# 核心策略：
#   - 每次只下载1个文件，避免同时抢带宽
#   - 每个文件使用4条连接分段下载
#   - 强制IPv4，绕开部分不稳定IPv6路径
#   - 无限重试并断点续传
#   - 自动接管旧脚本留下的 .part 文件
#
...
# 你的终端已经实测：
# curl.exe -I -x http://127.0.0.1:7899 https://ftp.ncbi.nlm.nih.gov/
# 返回 HTTP/1.1 200，因此7899代理已经确认可用。
# 本版本不再进行R内部代理测试，直接固定使用7899，避免误判。
PROXY_URL <- "http://127.0.0.1:7899"

# 每个文件的连接数。建议先用4；不要直接开16或32。
CONNECTIONS_PER_FILE <- 4L

# TRUE：包含GSE178265约4.7 GB的大矩阵
...
download_one <- function(geo, filename, url) {
  ...
  # -j1：一次只处理一个文件
  # -x/-s：单文件4连接
  # -c：接管curl/browser留下的顺序下载文件
  # disable-ipv6：避免部分不稳定IPv6国际链路
  args <- c(
    "--dir", dest_dir,
    "--out", part_name,
    "--continue=true",
...
# END 00A5_fast_GEO_download_fixed_7899(1).R


# 00B~00B_critical_metadata_audit_v2(1).R ----

#
# 00B_critical_metadata_audit_v2.R
# 项目：PD干细胞治疗“功能身份—疾病脆弱性悖论”
#
# 目的：
#   1. 解压并审计 GSE200610 的 CSV / barcode / clone 线索
#   2. 解压并逐个读取 GSE233885 的 RDS，确认 rat / timepoint /
#      projection target / retrograde barcode 等字段
#   3. 检查 GSE178265 是否已经放入外部细胞级 metadata
#
# 适用：
#   - Windows
#   - 枪神8 Plus，32 GB RAM
#   - 所有GEO主文件已经下载完成
#
# v2修复：显式关闭gz连接、修正NA误判、允许空结果写入Excel
#
# 内存原则：
#   - TAR 顺序解压
#   - GSE233885 每次只读取一个 RDS
#   - 读取后只保留 metadata，立刻删除完整对象并 gc()
#   - 不读取 GSE178265 4.74 GB 表达矩阵
#
...
# END 00B_critical_metadata_audit_v2(1).R


# 00C~00C_GSE178265_barcode_match_v2(2).R ----
...
# END 00C_GSE178265_barcode_match_v2(2).R


# 00D~00D_deep_label_forensic_audit(2).R ----

#
# 00D_deep_label_forensic_audit.R
# 项目：PD干细胞治疗“功能身份—疾病脆弱性—安全风险”纯生信项目
#
# 目的：
#   A. 深度抢救 GSE233885 的真实投射标签
#      - 检查 meta.data、Idents、cell names、feature names
#      - 检查 assays / misc / tools / reductions / commands 等对象结构
#      - 搜索 PFC、dlSTR、MNM008、mCherry、AAV、retro、WPRE 等线索
#
#   B. 深度抢救 GSE200610 的真实克隆映射
#      - 检查14个CSV的文件名、表头、第一列、非标准feature
#      - 搜索 clone、lineage、molecular barcode、viral barcode、LV/WPRE
#      - 搜索非10x型DNA序列，区分普通cell barcode与克隆barcode
#
#   C. 自动给出下一步决策
#      - 发现强证据：进入标签重建
#      - 只有弱线索：联系作者，同时启用替代模块
#      - 完全未发现：不伪造标签，正式切换替代模块
#
# 内存原则：
#   - GSE233885 每次只读取一个RDS
#   - 不同时保留多个Seurat对象
#   - 不读取或复制完整表达矩阵
#   - GSE200610只读取表头和第一列，不整表载入
#
# 适用环境：
#   - Windows
#   - 32 GB RAM
...
audit_one_g200_csv <- function(csv_path) {
  ...
  # 非标准feature候选：不是典型gene symbol/Ensembl，或命中病毒/条形码词
...
# END 00D_deep_label_forensic_audit(2).R


# 00E4~00E4_v2_GSE200610_plain_csv_candidate_validation(2).R ----

#
# 00E4_v2_GSE200610_plain_csv_candidate_validation.R
#
# 目的：
#   1. 只读取已经完全解压的14个普通.csv，不再直接读取.csv.gz
#   2. 审计列名、第一列feature名和潜在barcode/clone字段
#   3. 区分普通10x cell barcode、基因名伪阳性和潜在分子barcode
#   4. 明确判断公开矩阵能否恢复cell-to-clone映射
#
# 适用环境：Windows + 32 GB RAM
# 说明：采用2个并行worker；每个文件只读取表头和第一列。
#
...
if (nrow(candidate_table) > 0L) {
  ...
  # 在data.table的j表达式之外完成mapply，避免Windows/R环境中
  # 参数名与base::seq锁定绑定发生冲突
...
# END 00E4_v2_GSE200610_plain_csv_candidate_validation(2).R


# 01A~01A_FORMAL_PRODUCTION_IMPORT_ALL_IN_ONE_V2(2).R ----

#
# 01A_FORMAL_PRODUCTION_IMPORT_ALL_IN_ONE_V2.R
#
# 这是 01A 的正式生产版 V2（all-in-one）。
#
# 使用场景：
#   你完成 00E4 后，直接 Source 本脚本。
#   不需要再手动运行 R2A2 / R2A3 / R3-R11 那些补丁脚本。
#
# 本脚本整合了前面所有真实遇到的问题：
#   1. 普通 10x MatrixMarket 数据自动导入；
#   2. matrix.mtx.gz 损坏时，优先使用 02_recovered_mtx 里的 recovered.mtx；
#   3. features.tsv.gz 损坏 / 行数不对时，用同数据集可读 feature 模板自动修复；
#   4. GSE204796 的坏 features 自动修复；
#   5. GSE132758 的坏 matrix/features 自动处理；
#   6. GSE233885 的双层 / 异常 gzip rds 自动递归解压读取；
#   7. GSE157783 的 cell / genes / UMI dense matrix 流式转 sparse Seurat；
#   8. GSE204795 的 DESeqDataSet bulk 对象正式接收，不强行转 Seurat；
#   9. GSE178265 不读取全量超大矩阵，只准备 DA target barcodes 给 01B；
#  10. 最终统一输出 01A 状态表、失败表、metadata、summary 和报告。
#
# 成功标志：
#   最后显示：
#     ✅ 01A全局失败数量：0。
...
# FALSE：已有可读对象就跳过；适合正常使用
# TRUE ：强制重建所有 01A 对象；一般不要开
REBUILD_EXISTING <- FALSE

# GSE157783 流式读取 UMI 时每个 chunk 的 gene 行数。
# 内存紧张可改成 50；速度更快可改成 150/200。
GSE157783_CHUNK_N_GENES <- 100L

# MatrixMarket streaming chunk
    ...
    out[[i]] <- tryCatch(
      {
        ...
        # bulk 对象：例如 GSE204795 的 DESeqDataSet
...
#
# 5. GSE157783 专门导入：cell / genes / UMI dense matrix -> sparse Seurat
#
...
stream_dense_umi_to_sparse <- function(umi_path, genes, cells, chunk_n_genes = 100L, dataset_id = "GSE157783") {
  ...
  handle_line <- function(line) {
    ...
    # 允许 UMI 行数比 genes 少；如果多于 genes，就忽略多余行
...
#
# 6. GSE178265：只准备 DA target barcodes 给 01B
#
...
prepare_gse178265_da <- function(dataset_id, dataset_dir) {
  ...
  if (length(candidate_tables) == 0L) {
    # V2 修正：
    # 这不是 01A 的致命导入失败，而是 01B 之前需要进一步选择 DA target barcode。
    # 因此不再把 01A 判为 FAILED，避免正式 01A 被 annotation 命名差异卡死。
  ...
  # 如果 raw_barcodes 存在，保留能在 raw barcode 中直接或宽松匹配的 barcode
...
# 6B. 已有标准对象兜底
#
# 如果某个数据集的原始格式没有被自动识别，但 02_objects/01A_standardized/<GSE>
# 里已经存在可读对象，就不应该把它判为失败。
...
# unified sample metadata，主要收集 status 层面的 dataset/sample/object 信息
...
# END 01A_FORMAL_PRODUCTION_IMPORT_ALL_IN_ONE_V2(2).R


# 01B~01B_GSE178265_DA_STREAMING_SUBMATRIX_V7(2).R ----
...
# END 01B_GSE178265_DA_STREAMING_SUBMATRIX_V7(2).R


# 02A~02A_OBJECT_INTEGRITY_AND_QC_METRICS_V2(2).R ----
...
# END 02A_OBJECT_INTEGRITY_AND_QC_METRICS_V2(2).R


# 02B~02B_QC_VISUALIZATION_AND_FILTERING_DECISION(2).R ----
...
#   接在 02A 后面运行。
#
#   02A 已经计算 QC 指标，但不删细胞。
#   02B 做：
#     1. 读取 02A 的 QC summary / cell QC 表 / 02A Seurat objects
#     2. 为每个 Seurat object 自动生成保守 QC 阈值
#     3. 输出 QC 阈值表和过滤前后细胞保留表
#     4. 保存过滤后的 Seurat object 副本
#     5. 输出总体 QC 图和 per-object QC 图
#     6. 写 02B 报告
#
# 注意：
#   02B 不覆盖 01A / 01B / 02A 对象。
#   过滤后的对象会单独保存到：
#     D:/PD_Graft_Project/02_objects/02B_qc_filtered/
#
# 成功标志：
#   ✅ 02B QC visualization and filtering decision 完成。
#
...
# nFeature_RNA 下限：
#   普通 scRNA 常用 200。
#   如果某对象 median nFeature 很低，则自动降到 100，避免误删 snRNA/低深度样本。
NFEATURE_LOW_DEFAULT <- 200L
NFEATURE_LOW_LOWDEPTH <- 100L
LOW_DEPTH_MEDIAN_NFEATURE_CUTOFF <- 500L

# nFeature_RNA 上限：
#   默认用 object 内部 99.5% 分位数，主要去除极端 doublet-like outliers。
...
# percent.mt 上限：
#   如果对象里几乎没有 mitochondrial gene，则不按 mt 过滤。
#   如果有 mt 信号，则使用 max(15, p99)，并限制最高不超过 30。
...
#
# 6. 应用过滤，保存 02B filtered objects
#
...
# END 02B_QC_VISUALIZATION_AND_FILTERING_DECISION(2).R


# 03A~03A_NORMALIZATION_AND_PER_OBJECT_REDUCTION_V3_PCA_ONLY_RESUME(1).R ----
...
# END 03A_NORMALIZATION_AND_PER_OBJECT_REDUCTION_V3_PCA_ONLY_RESUME(1).R


# 03B~03B_MERGE_WITHIN_DATASET_AND_BATCH_CHECK_V6_JOINLAYERS_DIMNAMES_SAFE(1).R ----

#
# 03B_MERGE_WITHIN_DATASET_AND_BATCH_CHECK_V6_JOINLAYERS_DIMNAMES_SAFE.R
#
# 目的：
#   03B V3 仍有 3 个 dataset 失败，原因是 Seurat v5 layer / subset / merge
#   在某些对象上不稳定。
#
#   V4 改成“Matrix-only 安全版”：
#     1. 不再 subset Seurat object
#     2. 不再 merge Seurat object
#     3. 直接从每个对象的 counts matrix 抽样列
#     4. 用 sparse Matrix 手工 cbind 成 dataset-level counts
#     5. 再 CreateSeuratObject
#     6. 只做 Normalize / HVG / Scale / PCA
#
# 重要严谨性说明：
#   03B V4 的 downsampled merged PCA object 只用于 batch check / dataset-level QC。
#   它不是最终生物学结论对象。
#   最终 annotation / DEG / scoring / ML 必须回到 full filtered objects 或明确的 integration 主对象。
#
# 成功标志：
#   ✅ 03B V6 JoinLayers dimnames-safe matrix-only batch check 完成。
#
...
# 每个 object 最多抽样多少细胞用于 03B batch check
MAX_CELLS_PER_OBJECT_FOR_03B <- 500L

# 单 object dataset 如果细胞数大，也最多抽这么多；GSE178265_DA_01B 之前已成功，可跳过
...
get_counts_matrix_robust <- function(obj, assay) {
  # V6：JoinLayers + dimnames-safe 版本。
  # 03B V5 已经证明这些对象有 layers=counts,data,scale.data，
  # 但是 LayerData/GetAssayData 取出的 matrix 可能没有 dimnames，V5 因此误判为不可用。
  # V6 会自动补 rownames/colnames，并在必要时先 JoinLayers。
  ...
  # 最后兜底：用 data layer 做 batch check。这不是最终表达量分析，只用于 03B PCA inspection。
...
#
# 5. 按 dataset 做 matrix-only downsample merge
#
...
for (ds in datasets) {
  ...
  for (i in seq_len(nrow(ds_manifest))) {
    ...
    if (nrow(meta) > 0L && all(keep_cells %in% rownames(meta))) {
      ...
      # V5：如果 layer cell names 与 meta rownames 对不上，保留最核心 metadata。
  ...
  # 如果 do.call(cbind2) 在某些 Matrix 版本失败，备用 base cbind
...
# END 03B_MERGE_WITHIN_DATASET_AND_BATCH_CHECK_V6_JOINLAYERS_DIMNAMES_SAFE(1).R


# 03C~03C_MAIN_ANALYSIS_STRATEGY_AND_MANIFEST(1).R ----
...
# END 03C_MAIN_ANALYSIS_STRATEGY_AND_MANIFEST(1).R


# 04A~04A_MARKER_PANEL_AND_ANNOTATION_PREP(1).R ----
...
# END 04A_MARKER_PANEL_AND_ANNOTATION_PREP(1).R


# 04B~04B_MARKER_EXPRESSION_AND_PRELIMINARY_ANNOTATION_V4_DIRECT_MATRIX_LISTFIX(1).R ----

#
# 04B_MARKER_EXPRESSION_AND_PRELIMINARY_ANNOTATION_V4_DIRECT_MATRIX_LISTFIX.R
#
# 目的：
#   修复 04B V3 的脚本变量作用域 bug：
#     找不到对象 'matrix_source_list'
#
#   V3 已经证明直接 matrix extraction 是成功的：
#     matrix：xxxxx genes x xxxxx cells；method=direct_Assay5_layers；layer=counts
#
#   V4 保留 V3 的底层 matrix 提取逻辑，但改成：
#     1. 每个 object 返回 result list
#     2. 在 tryCatch 外面统一 append 到总 list
#     3. 不再使用 <<- 修改外层对象
#     4. 避免 source/local 环境导致变量找不到
#
# 严谨性：
#   04B V4 仍然只是 preliminary marker-supported suggestion。
#   最终 annotation 需要 04C/04D 验证。
#
# 成功标志：
#   ✅ 04B V4 direct-matrix marker expression 完成。
#
...
# END 04B_MARKER_EXPRESSION_AND_PRELIMINARY_ANNOTATION_V4_DIRECT_MATRIX_LISTFIX(1).R


# 04C~04C_REVIEW_MARKER_EXPRESSION_AND_DEFINE_ANNOTATION_RULES(1).R ----
...
#   接在 04B V4 后运行。
#
#   04B 已经成功提取 marker expression：
#     - 53 / 54 objects 成功
#     - 1 个 reference/auxiliary object 失败可记录
#
#   04C 做“人工审阅前的规则化整理”：
#     1. 汇总 preliminary annotation suggestion
#     2. 汇总每个 dataset 的 marker-category signal
#     3. 提取 DA-like / A9-like / A10-like / progenitor / safety-risk 候选 group
#     4. 输出 annotation rule table
#     5. 输出需要人工重点检查的 group
#
# 注意：
#   04C 仍然不是最终 annotation。
#   它是把 04B 的 marker evidence 整理成可审阅表。
#   最终标签要在 04D 才写回对象。
#
# 成功标志：
#   ✅ 04C marker review and annotation rules 完成。
#
...
# marker category 被认为有信号的最低要求
MIN_COVERAGE <- 0.4
MIN_MEAN_SCORE <- 0.05
MIN_PCT_CELLS_SCORE_GT0 <- 0.05

# 高优先级 review groups 最多输出多少行
...
wide_category_scores <- function(dt) {
  # 输入必须有 dataset/object_id/group_id/category/mean_score/pct_cells_score_gt0/coverage_fraction
...
#
# 7. Dataset-level category signal summary
#
...
#
# 8. Candidate groups for manual review
#
...
#
# 10. Safety-risk candidate table
#
...
# END 04C_REVIEW_MARKER_EXPRESSION_AND_DEFINE_ANNOTATION_RULES(1).R


# 04D~04D_V3_FINAL_AUDIT_AND_REPAIR_MISSING_ANNOTATED_OBJECTS(1).R ----
...
# END 04D_V3_FINAL_AUDIT_AND_REPAIR_MISSING_ANNOTATED_OBJECTS(1).R


# 05A~05A_DA_A9_A10_AND_PROJECTION_COMPETENCE_SCORING(1).R ----
...
#   05A 是第一个核心创新模块：
#     DA-like identity
#     A9-like / A10-like molecular bias
#     Projection-associated molecular competence
#
#   重要：
#     这里的 projection competence 是“投射相关分子能力”。
#     不能写成真实 retrograde projection，也不能声称已经证明轴突真实投射。
#
# 输入：
#   04D_annotated_object_manifest.csv
#
# 输出：
#   05A_signature_gene_sets.csv
#   05A_cell_level_scores.csv
#   05A_group_level_scores.csv
#   05A_object_level_scores.csv
#   05A_DA_A9_A10_projection_candidate_groups.csv
#   05A_failed_objects.csv
#   05A report
#
# 成功标志：
#   ✅ 05A DA/A9/A10/projection competence scoring 完成。
#
...
# 是否输出 cell-level 大表。约 20 万行，可以输出。
...
signature_sets <- list(
  ...
  projection_associated_molecular_competence = c(
    # neurite/axon maturation
    "STMN2", "GAP43", "DCX", "MAP2", "NEFL", "NEFM",
    # synapse / vesicle release
    "SNAP25", "SYT1", "SYN1", "RAB3A", "STX1A",
    # adhesion and axon guidance / pathfinding
...
for (i in seq_len(nrow(manifest))) {
  ...
  tryCatch({
    ...
    # matrix source and coverage audit
    cov[, dataset := ds]
    cov[, object_id := oid]
...
# END 05A_DA_A9_A10_AND_PROJECTION_COMPETENCE_SCORING(1).R


# 05A_V2~05A_V2_FINAL_AUDIT_AND_FAILURE_RECORD(1).R ----
...
# END 05A_V2_FINAL_AUDIT_AND_FAILURE_RECORD(1).R


# 05B~05B_SAFETY_RISK_SCORING_AND_CONTRAST(1).R ----
...
# END 05B_SAFETY_RISK_SCORING_AND_CONTRAST(1).R


# 06A~06A_FIGURE_TABLE_PREP_DA_PROJECTION_SAFETY(1).R ----
...
# END 06A_FIGURE_TABLE_PREP_DA_PROJECTION_SAFETY(1).R


# 06B~06B_V2_PUBLICATION_FIGURE_POLISH_DA_PROJECTION_SAFETY(1).R ----

#
# 06B_V2_PUBLICATION_FIGURE_POLISH_DA_PROJECTION_SAFETY.R
#
# 目的：
#   修复 06B 第一版图的问题：
#     1. Figure2A 标签被截断
#     2. Figure2C / 2D legend 太长
#     3. Figure2E top story heatmap 行太多、过挤
#     4. 输出更适合放进 manuscript draft 的 Figure 2A–E V2
#
# 输入：
#   06A_figure_table_prep 下的表格
#
# 输出：
#   D:/PD_Graft_Project/04_figures/06B_publication_figure_drafts_V2/
#   D:/PD_Graft_Project/03_tables/06B_publication_figure_drafts_V2/
#
# 成功标志：
#   ✅ 06B V2 publication figure polish 完成。
#
...
# Figure 2A: scatter with margins
...
# Figure 2C: candidate class proportion
if (nrow(class_prop) > 0) {
  p2c <- ggplot(
    class_prop,
    aes(
...
# Figure 2E: tile plot of top story groups
if (nrow(tile_dt) > 0) {
  tile_long <- melt(
    tile_dt,
    id.vars = c(
...
# END 06B_V2_PUBLICATION_FIGURE_POLISH_DA_PROJECTION_SAFETY(1).R


# 06C~06C_MANUSCRIPT_RESULTS_TEXT_DRAFT(1).R ----
...
#
#   06C 不做新计算。
#   06C 根据 05A/05B/06A/06B 的结果，自动生成：
#     1. manuscript-style Results draft（英文）
#     2. 中文解释版 Results draft
#     3. Figure 2 legend draft
#     4. key claims and caution table
#     5. result summary report
#
# 重要严谨性：
#   不能写 real projection
#   不能写 proven functional integration
#   不能写 proven tumorigenicity / proven safety
#
#   只能写：
#     projection-associated molecular competence
#     safety-risk-associated transcriptional state
#     DA-like / A9-like / A10-like molecular bias
#
# 成功标志：
#   ✅ 06C manuscript results text draft 完成。
#
...
# Prefer V2 figure index if available
...
# A9/A10 leading bias per dataset
...
# Candidate class leading class per dataset
...
# END 06C_MANUSCRIPT_RESULTS_TEXT_DRAFT(1).R


# 06D~06D_DISCUSSION_ABSTRACT_AND_MANUSCRIPT_STRUCTURE_DRAFT(1).R ----

#
# 06D_DISCUSSION_ABSTRACT_AND_MANUSCRIPT_STRUCTURE_DRAFT.R
#
# 目的：
#   接在 06C 后运行。
#
#   06D 不做新计算。
#   06D 根据 06C 的 Results 草稿和前面 key tables 自动生成：
#     1. Abstract draft 英文版
#     2. 中文摘要解释版
#     3. Discussion draft 英文版
#     4. 中文 Discussion 解释版
#     5. Manuscript structure / Figure plan
#     6. Limitations and safe wording table
#
# 重要严谨性：
#   不能写：
#     real projection
#     retrograde projection
#     proven functional integration
#     proven tumorigenicity
...
#
# 成功标志：
#   ✅ 06D discussion, abstract and manuscript structure draft 完成。
#
...
#
# 6. Limitations and safe wording
#
...
#
# 11. Manuscript structure and figure plan
#
...
# END 06D_DISCUSSION_ABSTRACT_AND_MANUSCRIPT_STRUCTURE_DRAFT(1).R


# 07A~07A_ML_DATASET_PREPARATION_V3_CONTRAST_BASED(1).R ----

#
# 07A_ML_DATASET_PREPARATION_V3_CONTRAST_BASED.R
#
# 目的：
#   修复 07A V2 出现 training groups = 0 的问题。
#
# 原因：
#   V2 以 04B marker_wide 作为 master，再 merge 05A/05B。
#   由于不同表中的 group_id / group_source 命名不完全一致，05B label 没有成功 merge 到 master，
#   导致 ideal/safety marker-rule-derived labels 全部 NA，训练表变成 0 行。
#
# V3 修复策略：
#   1. 直接以 05B_DA_projection_vs_safety_contrast_groups.csv 作为 ML master base
#      因为这个表已经成功包含 DA/projection + safety contrast + class label。
#   2. 再尝试 merge 04B marker-category wide features。
#   3. 如果 marker merge 不完全，也不阻塞 ML 数据集生成。
#   4. 输出可训练的 ideal / safety marker-rule-derived training tables。
#
# 严谨性：
#   label 是 rule-derived marker-rule-derived label，不是实验 ground truth。
#   如果使用 label-defining composite features 训练模型，只能作为 exploratory marker-rule-derived model。
#
# 成功标志：
#   ✅ 07A V3 contrast-based ML dataset preparation 完成。
#
...
if (!"n_cells" %in% colnames(master)) {
  # 05B contrast 有时叫 n_cells_05B 或 n_cells_05A
...
# 05B contrast 里通常已有 05A/05B 关键 scores：
needed_scores <- c(
  "DA_like_composite_score",
  "projection_competence_composite_score",
  "DA_projection_competence_composite_score",
...
#
# 6. 尝试 merge 04B marker category features
#
...
#
# 7. 定义 marker-rule-derived labels
#
...
# Full descriptive feature set includes direct composite scores.
# Primary non-leakage feature set excludes direct label-defining composites, but may be sparse if marker merge incomplete.
leakage_cols <- c(
  "DA_like_composite_score",
  "projection_competence_composite_score",
  "DA_projection_competence_composite_score",
...
# 如果 primary features 太少，就保留 marker + component features，仍然记录 leakage warning
...
#
# 10. Feature dictionary / label definitions
#
...
# END 07A_ML_DATASET_PREPARATION_V3_CONTRAST_BASED(1).R


# 07B~07B_TRAIN_EXPLORATORY_WEAK_LABEL_ML_MODELS(2).R ----
...
# END 07B_TRAIN_EXPLORATORY_WEAK_LABEL_ML_MODELS(2).R


# 08A~08A_V19_MEMORY_SAFE_BALANCED_PUBLICATION_PDF(2).R ----

#
# 08A_V19_MEMORY_SAFE_BALANCED_PUBLICATION_PDF.R
#
# 目的：
#   最终修正版完整 08A。
#
# 修复 V17 问题：
#   1. 只生成了少量图：V18 每张图单独 tryCatch，单张失败不影响同对象其他图，也不影响其他 dataset。
#   2. 点太淡：V18 增大点大小，加深颜色，连续 score 使用更清楚的 viridis 色阶。
#   3. 仍然只输出 PDF，不输出 PNG。
#   4. 最终 PDF 内嵌高清 raster 图层，避免 vector PDF 黑块/竖线/大色块。
#
# V18 输出：
#   每个代表对象尽量输出：
#     1. cluster UMAP
#     2. 04D conservative annotation UMAP
#     3. 05B DA/projection-associated class UMAP
#     4. DA-like score UMAP
#     5. projection-associated competence score UMAP
#     6. DA/projection-associated composite score UMAP
#     7. A9-minus-A10 score UMAP
#     8. safety-risk-associated score UMAP
#
# 严谨性：
#   UMAP 是 visualization。
#   04D annotation 是 conservative marker-supported transcriptomic label。
#   05B class 是 rule-derived candidate state。
#   projection-associated score 不证明真实解剖投射。
#   safety-risk score 不证明肿瘤形成或临床安全性。
#
# 成功标志：
#   ✅ 08A V19 memory-safe balanced publication PDF complete UMAP 完成。
#
...
# 只输出 PDF；中间高清 PNG 自动删除
PDF_WIDTH <- 9.2
PDF_HEIGHT <- 6.2

# V19 memory-safe balanced setting:
# V18 的 450 dpi / 4140x2790 对大对象容易让 RStudio abort。
# V19 降到 300 dpi 左右的视觉质量，同时显著降低内存占用。
RENDER_DPI <- 300
RENDER_WIDTH_PX <- 2760
RENDER_HEIGHT_PX <- 1860

# 点大小：比 V17/V18 更粗，避免看起来太淡；但不使用低清像素图。
POINT_SIZE_DISCRETE <- 0.58
POINT_SIZE_CONTINUOUS <- 0.60
CLUSTER_LABEL_SIZE <- 3.2

# 如果有对象细胞数非常多，仍全部画；这里不下采样
...
open_highres_png_device <- function(filename) {
  # V19: 不用 ragg，避免额外内存开销；直接用 grDevices PNG device。
  grDevices::png(
    filename = filename,
...
# END 08A_V19_MEMORY_SAFE_BALANCED_PUBLICATION_PDF(2).R


# 08B~08B_FINAL_V3_FIX_HEATMAP_COLORBAR_LAYOUT(1).R ----
...
# END 08B_FINAL_V3_FIX_HEATMAP_COLORBAR_LAYOUT(1).R


# 08C~08C_JOURNAL_STANDARD_ALL_FILTERED_GENES_CHUNKED_DEG_VOLCANO(1).R ----
...
# END 08C_JOURNAL_STANDARD_ALL_FILTERED_GENES_CHUNKED_DEG_VOLCANO(1).R


# 08D1~08D1_GO_FINAL_VERIFIED_V2_MINIMAL_SAFE(1).R ----
...
# END 08D1_GO_FINAL_VERIFIED_V2_MINIMAL_SAFE(1).R


# 08D2~08D2_KEGG_FINAL_UP_DOWN_DOTPLOT(1).R ----

#
# 08D2_KEGG_FINAL_UP_DOWN_DOTPLOT.R
#
# 08D2：KEGG only - FINAL publication candidate
#
# 目的：
#   只做 KEGG enrichment + 你原来脚本风格的 UP/DOWN 合并 dotplot。
#   不包含 GO。
#
# 设计：
#   - KEGG UP / DOWN 放在同一张 dotplot
#   - 红线：Up-regulated
#   - 绿线：Down-regulated
#   - x = GeneRatio
#   - y = KEGG pathway name
...
#
# 方法边界：
#   - KEGG ORA 使用 clusterProfiler::enricher + MSigDB C2 CP:KEGG gene sets
#   - universe = 08C 中所有通过客观表达过滤并实际被测试的 genes
#   - UP = ideal-like 方向上调 genes
#   - DOWN = lower-priority/mixed 方向上调 genes
#   - 这一步是 pathway-level interpretation，不是功能验证或临床证明
#
# 输出：
#   D:/PD_Graft_Project/03_tables/08D2_KEGG_FINAL/
#   D:/PD_Graft_Project/04_figures/08D2_KEGG_FINAL_pdf/
#
# 成功标志：
#   ✅ 08D2 KEGG FINAL 完成。
#
...
# 与 08C/08D1 保持一致的 DEG list cutoff
DEG_PADJ_CUTOFF <- 0.05
DEG_LOG2FC_CUTOFF <- 0.25

# enrichr 内部不要提前截断，所有结果先输出，正式作图再按 PLOT_PADJ_CUTOFF 过滤
ENRICH_PVALUE_CUTOFF <- 1
ENRICH_QVALUE_CUTOFF <- 1

# 正式 KEGG 图只画 FDR <= 0.05 的 pathway
PLOT_PADJ_CUTOFF <- 0.05
...
save_pdf_plot <- function(plot_obj, file_path, width, height) {
  ...
  # 避免旧 PDF 被打开导致写入失败：如果删不掉，直接报清楚原因
  if (file.exists(file_path)) {
...
#
# 7. clusterProfiler::enricher KEGG enrichment
#
...
# END 08D2_KEGG_FINAL_UP_DOWN_DOTPLOT(1).R


# 08E~08E_HALLMARK_GSEA_FINAL_PUBLICATION_V4_FGSEA_SERIAL(2).R ----
...
# END 08E_HALLMARK_GSEA_FINAL_PUBLICATION_V4_FGSEA_SERIAL(2).R
