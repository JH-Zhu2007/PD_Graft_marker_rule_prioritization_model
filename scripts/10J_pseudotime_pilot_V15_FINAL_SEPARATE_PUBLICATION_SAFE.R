############################################################
# 10J_pseudotime_pilot_V15_FINAL_SEPARATE_PUBLICATION_SAFE.R
# Project: PD_Graft_Project
# Purpose:
#   Final safe, separate, publication-style 10J pseudotime pilot.
#   - One script, standalone.
#   - Base R plotting only; no ggplot2.
#   - Separate PDFs, not one-page composite.
#   - Conservative PDF device/margins to avoid blank plots and
#     "figure margins too large".
#   - No raw-cell scatter in trend panels; only binned median/IQR.
#   - Explicit claim boundary: graph-based transcriptomic pseudotime
#     pilot, not true lineage tracing.
############################################################
...
# Close any orphan graphics device to avoid device-state contamination.
...
safe_pdf <- function(filename, width, height, plot_fun) {
  ok <- FALSE
  err <- NA_character_
  # Always close existing non-null device before opening a new one.
  while (grDevices::dev.cur() > 1) {
    try(grDevices::dev.off(), silent = TRUE)
  }
  tryCatch({
...
# ------------------------- graph pseudotime -------------------------
# Root cells = lowest priority-proxy cells (bottom quantile), fallback to high stress/progenitor.
...
# Pure numeric graph to avoid vertex-name/NUL PDF/device issues.
...
# Multi-root shortest-path distance.
dmat <- tryCatch(
...
# disconnected cells: impute max finite
...
# Orient pseudotime to positive priority proxy direction when possible.
...
# Cluster median spread for decision
...
safe_pdf(fig_B, width = 6.5, height = 5.4, function() {
  ...
  # Compact text legend, no colorbar rectangles to avoid device artifacts.
...
# D: program trend panels, no raw scatter
...
# E: priority proxy, no raw scatter
...
# F: marker trend heatmap, safe and compact
marker_order <- c(
...
if (!is.null(heat_z) && nrow(heat_z) >= 5 && ncol(heat_z) >= 2) {
  safe_pdf(fig_F, width = 5.9, height = 7.7, function() {
    ...
    # image expects matrix as x by y; reverse rows for top-to-bottom marker display.
    ...
    # Compact color key as text to avoid extra plot region/margin problems.
