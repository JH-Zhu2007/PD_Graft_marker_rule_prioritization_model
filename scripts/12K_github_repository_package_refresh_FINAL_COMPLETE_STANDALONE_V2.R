...
# ============================================================
# 12K FINAL COMPLETE STANDALONE V2
# GitHub / repository package refresh with workflow-script indexing repair
# for DA neuron / graft-related transcriptomic cell-state prioritisation framework
#
# User rule respected:
#   - Complete standalone script for 12K
#   - Does NOT read any previous 12K output
#   - Does NOT patch old 12K tables or figures
#   - May read locked upstream outputs as formal inputs:
#       10A-10P, 11A-11J, 12A, 12B, 12C, 12D, 12E, 12F, 12G, 12H, 12I, 12J
#   - Uses 12J final Methods/reproducibility handoff as formal input
#   - Repairs V1 issue: workflow script index rows were 0
#   - Searches scripts in D:/PD_Graft_Project/01_scripts and D:/PD_Graft_Project
#     and captures the currently sourced 12K V2 script path from sys.frames()
#   - Generates GitHub-ready package structure, README, script index,
#     provenance manifests, reproducibility checklist, claim-boundary statement,
#     code availability statement, and 12L journal/cover-letter handoff
#   - No internet
#   - No 00-10P rerun
#
# Scientific boundary:
#   - Repository/package documentation only
...
# Hard rule: do not read previous 12K output
...
# ------------------------- read locked 12J and upstream inputs -------------------------
...
# Also write key provenance tables directly inside repo package
...
# 1) Normal intended location
if (dir.exists(script_root)) {
  script_files <- c(script_files, list.files(script_root, pattern = "\\.(R|r)$", recursive = TRUE, full.names = TRUE))
}

# 2) Broader project search; exclude obvious data/object/output-heavy folders but keep actual scripts anywhere under project
...
# 3) Current sourced script from temporary download path
...
if (length(script_files) > 0) {
  ...
  # Copy all discovered script files into scripts/, preserving basename with de-dup suffix if needed.
...
# Add an explicit warning/note table so the GitHub package is honest if only the current script is captured.
...
# ------------------------- figures -------------------------
# FigA repository package overview
...
# FigC package provenance readiness
...
# FigD claim-boundary package audit
