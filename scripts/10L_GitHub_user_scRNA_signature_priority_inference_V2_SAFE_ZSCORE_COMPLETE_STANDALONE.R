#!/usr/bin/env Rscript
# ==============================================================================
# 10L_user_scRNA_frozen_predictor_inference_V2_SAFE_ZSCORE_COMPLETE_STANDALONE.R
# ------------------------------------------------------------------------------
# Purpose:
#   Apply the locked 09C reduced-feature marker-rule-derived model package, when a
#   serialized model is available, to a user-provided scRNA/Seurat object.
#   If no usable serialized 09C model is detected, the script DOES NOT pretend
#   to run ML. It falls back to a clearly labelled frozen signature-priority
#   inference mode and writes this limitation to all reports.
#
# Main outputs:
#   1) model and feature readiness audit
#   2) per-cell inference table
#   3) per-cluster inference summary
#   4) gene-set and model-feature coverage reports
#   5) optional diagnostic PDFs
#
# Claim boundary:
#   This module returns transcriptomic prioritization outputs only.
#   It is NOT a clinical-use model, NOT a graft efficacy predictor,
#   NOT a tumorigenicity/safety predictor, and NOT a lineage-tracing assay.
#
# Recommended project flow:
#   10G -> 10H -> 10I -> 10J -> 10K -> 10L -> 10M
# ==============================================================================
...
# Set this to a new user scRNA/Seurat object when using the module as a tool.
# Leave NA to run a built-in demo on the 10I-recommended GSE204796 D8 object.
USER_INPUT_RDS <- NA_character_

# Environment override is supported. Example in R:
# Sys.setenv(PD_GRAFT_USER_SCRNA_RDS = "D:/path/to/new_seurat.rds")
...
MAX_CELLS_FOR_INFERENCE <- Inf      # set e.g. 30000 for very large user objects
MAX_MODEL_FILE_MB       <- 750      # only attempt loading model RDS/RData under this size
MAKE_FIGURES            <- TRUE     # set FALSE if you only want tables/text outputs

# Output control. Cell-level table can be large; gzip is off for Windows friendliness.
...
zscore_vec <- function(x) {
  # Safe manual z-score. Avoid stats::sd()/var() because some Windows/RStudio
  # sessions can trigger locked-binding errors around the internal argument `use`.
  x <- safe_numeric(x)
  ok <- is.finite(x)
...
# This script intentionally uses base PDF only.
...
# ------------------------------
# 5. Load candidate model objects safely
# ------------------------------
...
# ------------------------------
# 6. Read input object and extract expression/metadata
# ------------------------------
...
# Align meta with expression columns
...
# Optional downsample for huge inputs
...
# ------------------------------
# 7. Gene sets and deterministic signature-priority scores
# ------------------------------
# These are locked, conservative marker sets for transcriptomic prioritization.
gene_sets <- list(
...
match_genes <- function(query_genes, matrix_rownames) {
  # Return actual matrix row names matching query genes, case-insensitive.
  # V1 accidentally used the logical mask on rn instead of the matched indices.
  rn <- matrix_rownames
  rn_upper <- toupper(rn)
...
score_gene_set <- function(mat, genes) {
  ...
  # Manual gene-wise z-score across cells. No apply(), no t(), no stats::sd().
  dense <- as.matrix(sub)
...
# Replace all-NA scores with 0 but record this in coverage table.
...
# ------------------------------
# 8. Try serialized frozen model predictions, if possible
# ------------------------------
...
make_newdata_for_model <- function(features, mat) {
  ...
  # create cell x feature data.frame; absent features are zero
...
extract_numeric_prediction <- function(pred, task_hint = "") {
  # ranger object sometimes returns a list
  ...
  if (is.matrix(pred) || is.data.frame(pred)) {
    ...
    cn <- colnames(p)
    if (!is.null(cn)) {
      # pick a positive/ideal/risk-like class if available, otherwise last numeric column
...
# ------------------------------
# 9. Build per-cell and cluster-level output
# ------------------------------
per_cell <- data.frame(
  cell_id = cell_ids,
...
# Rank cells by the most reliable score available: actual model prediction if any, otherwise proxy.
model_pred_names <- names(model_prediction_cols)
if (length(model_pred_names) > 0) {
  # average successful model predictions after z-scaling as a conservative ensemble
...
# ------------------------------
# 10. Optional diagnostic figures
# ------------------------------
if (MAKE_FIGURES) {
  # A. embedding priority, if embedding exists
  if (!is.null(emb)) {
  ...
  # C. signature heatmap per cluster
  ...
  # Safe column-wise z-score without apply(), avoiding locked-binding issues.
