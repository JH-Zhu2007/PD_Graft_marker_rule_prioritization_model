################################################################################
# 10H_dataset_role_and_model_scope_freeze_V1.R
# Project: PD_Graft_Project -> reframed as dopaminergic neuron / graft cell-state
# Purpose:
#   1) Read the completed 10G V8 standalone strict dataset-domain audit if present.
#   2) Freeze dataset roles and model scope in a clean, teacher-defensible table.
#   3) Separate core 09C model-development datasets from external/context/bulk/background.
#   4) Produce safe wording for project presentation and downstream 10I pseudotime planning.
#
# Important:
#   - This script DOES NOT retrain any model.
#   - This script DOES NOT modify/delete 10D/10E/10F figure packages.
#   - This script DOES NOT run pseudotime.
#   - This script only creates audit tables and explanatory text files.
################################################################################
...
# -----------------------------------------------------------------------------
# 2. Read completed 10G V8 outputs if available
# -----------------------------------------------------------------------------
...
# Known expected/project-observed accessions from completed 10G V8 console output.
# These are used as a fallback and to keep the lock table complete even if a CSV
# column name differs.
...
# Pull accession list from V8 CSVs if possible.
...
# Also robustly parse any GSE accession from all character columns if standard col absent.
for (df in list(v8_audit, v8_detected, v8_evid_sum)) {
...
# Explicitly remove known short false positives if ever present.
...
# -----------------------------------------------------------------------------
# 3. Manual role-lock rules
# -----------------------------------------------------------------------------
# This table intentionally freezes roles conservatively.
# Unknown/extra/historical accessions are NOT allowed into core model by default.
...
# Add rows for any newly detected accessions not in rules.
...
# Keep only accessions detected/expected in the current project scope.
...
# -----------------------------------------------------------------------------
# 4. Add evidence counts from 10G V8 if available
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 7. Output teacher-facing text files
# -----------------------------------------------------------------------------
