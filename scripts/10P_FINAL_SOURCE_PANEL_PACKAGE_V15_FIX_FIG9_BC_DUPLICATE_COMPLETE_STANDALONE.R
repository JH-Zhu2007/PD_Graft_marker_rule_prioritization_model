# ================================================================
# 10P_FINAL_SOURCE_PANEL_PACKAGE_V15_FIX_FIG9_BC_DUPLICATE_COMPLETE_STANDALONE.R
# Project: PD_Graft_Project
# Purpose:
#   Final 10P source-panel package fix after manual review.
#   Specifically fixes the last known issue: Figure 9B and Figure 9C are duplicate.
#
# Critical boundaries:
#   - No analysis rerun.
#   - No model retraining.
#   - No multi-panel assembly.
#   - No A/B/C labels added.
#   - No PNG/TIFF/JPG output.
#   - Only copies/filters individual source-panel PDFs.
#   - If Figure 9B and 9C are duplicates, remove 9C from MAIN and archive it as diagnostic duplicate.
#
# Final policy:
#   Figure 9 should NOT keep duplicate panels just to force a 3-panel figure.
#   If only two non-duplicate, content-worthy Figure 9 panels remain, Figure 9 is a 2-panel main figure.
# ================================================================
...
# -----------------------------
# Preferred input: final V14 package generated by the previous script.
# Fallbacks are included only so the script gives an informative error if V14 was not run.
...
# -----------------------------
# Copy input main/supp PDFs to V15 package
# -----------------------------
...
# Explicit duplicate test: 9B and 9C have same md5.
...
# Generic duplicate cleanup within Figure 9: keep first occurrence of each md5, remove later duplicates.
...
# -----------------------------
# Rebuild all-folder after duplicate removal
# -----------------------------
...
# -----------------------------
# Final duplicate audit across MAIN only and Figure 9 only
# -----------------------------
final_main <- list.files(out_main_dir, pattern = "\\.pdf$", full.names = TRUE, recursive = FALSE)
final_supp <- list.files(out_supp_dir, pattern = "\\.pdf$", full.names = TRUE, recursive = FALSE)
...
# Manual confirmation table: only non-empty if true duplicates remain.
